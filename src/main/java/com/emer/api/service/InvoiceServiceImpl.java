...
public class InvoiceServiceImpl implements InvoiceService {
	...
	public List<Invoice> searchInvoices(DateSearch dateSearchParams) {
		...
		if (isFullSearch(dateSearchParams)) {
			invoices = invoiceDao.findByCustomerIdAndIsPaidAndInvoiceDateBetween(dateSearchParams.getCustomerId(),
					dateSearchParams.getPaidStatus(), dateSearchParams.getDateFrom(), dateSearchParams.getDateTo());
			return determineCustomerDetails(invoices);
		}
		if (isSearchTimePeriodForStatus(dateSearchParams)) {
			invoices = invoiceDao.findByIsPaidAndInvoiceDateBetween(dateSearchParams.getPaidStatus(),
					dateSearchParams.getDateFrom(), dateSearchParams.getDateTo());
			return determineCustomerDetails(invoices);
		}
		if (isSearchTimePeriodForCustomer(dateSearchParams)) {
			invoices = invoiceDao.findByCustomerIdAndInvoiceDateBetween(dateSearchParams.getCustomerId(),
					dateSearchParams.getDateFrom(), dateSearchParams.getDateTo());
			return determineCustomerDetails(invoices);
		}
		if (isSearchTimePeriod(dateSearchParams)) {
			invoices = invoiceDao.findByInvoiceDateBetween(dateSearchParams.getDateFrom(),
	...
	public Invoice saveInvoice(NewInvoiceRequest newInvoiceRequest)
		...
		Invoice calculatedInvoice = this.calculateInvoice(newInvoiceRequest.getNewInvoice());

		if (newInvoiceRequest.isSendMail()) {
			this.mailInvoice(calculatedInvoice,
					this.customerService.findByCustomerId(calculatedInvoice.getCustomerId()));
		}

		return this.invoiceDao.save(calculatedInvoice);
	}
	 ...
	 */
	@Override
	public Invoice calculateInvoice(Invoice newInvoice) {
	...
	private boolean isFullSearch(DateSearch dateSearchParams) {
		return dateSearchParams.getPaidStatus() != null && dateSearchParams.getCustomerId() != null
	...
	private boolean isSearchTimePeriodForCustomer(DateSearch dateSearchParams) {
		return dateSearchParams.getCustomerId() != null && dateSearchParams.getDateFrom() != null
				&& dateSearchParams.getDateTo() != null;
	}
	...
	private List<Invoice> determineCustomerDetails(Iterable<Invoice> invoices) {
		...
		StreamSupport.stream(invoices.spliterator(), false).map(x -> x.getCustomerId()).collect(Collectors.toList())
				.stream().forEach(id -> customerDetails.put(id, this.getSalonName(id)));

		return StreamSupport.stream(invoices.spliterator(), false)
				.map(x -> this.updateInvoiceWithSalonName(x, customerDetails)).collect(Collectors.toList());
	}
